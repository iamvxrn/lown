...
func runInstallScript(appRoot, scriptName string, m *manifest.Manifest) error {
	...
	env := os.Environ()
	env = append(env,
		fmt.Sprintf("LOWN_BIN=%s", binDir),
		fmt.Sprintf("LOWN_APP_ROOT=%s", appRoot),
		fmt.Sprintf("LOWN_PACKAGE_NAME=%s", m.Package.Name),
		fmt.Sprintf("LOWN_PACKAGE_VERSION=%s", m.Package.Version),
		fmt.Sprintf("LOWN_EXECUTABLE=%s", m.GetExecutable()),
	)
	cmd.Env = env
...
func RunUninstallScript(appRoot, scriptName string, m *manifest.Manifest) error {
	...
	env := os.Environ()
	env = append(env,
		fmt.Sprintf("LOWN_BIN=%s", path.BinDir()),
		fmt.Sprintf("LOWN_APP_ROOT=%s", appRoot),
		fmt.Sprintf("LOWN_PACKAGE_NAME=%s", m.Package.Name),
		fmt.Sprintf("LOWN_PACKAGE_VERSION=%s", m.Package.Version),
		fmt.Sprintf("LOWN_EXECUTABLE=%s", m.GetExecutable()),
